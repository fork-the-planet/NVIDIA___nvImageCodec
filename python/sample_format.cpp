...
void SampleFormat::exportToPython(py::module& m)
{
    ...
    py::enum_<nvimgcodecSampleFormat_t>(m, "SampleFormat", "Enum representing sample format for image. Sample format describes how color components are matched to channels in given order and channels are matched to planes.")
        ...
        .value("P_AUTO_COMPONENTS", NVIMGCODEC_SAMPLEFORMAT_P_AUTO_COMPONENTS, "Planar layout; let color_spec choose the components. Preferred over P_UNCHANGED when color_spec is concrete.")
        .value("I_AUTO_COMPONENTS", NVIMGCODEC_SAMPLEFORMAT_I_AUTO_COMPONENTS, "Interleaved layout; let color_spec choose the components. Preferred over I_UNCHANGED when color_spec is concrete.")
        .value("P_UNCHANGED", NVIMGCODEC_SAMPLEFORMAT_P_UNCHANGED, "Unchanged planar format (alias for P_AUTO_COMPONENTS).")
        .value("I_UNCHANGED", NVIMGCODEC_SAMPLEFORMAT_I_UNCHANGED, "Unchanged interleaved format (alias for I_AUTO_COMPONENTS).")
